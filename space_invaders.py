...
# Level Up Text Duration
level_up_time = None
level_up_duration = 2000  # Duration in milliseconds
...
# Level Up Text Function
def level_up_text():
    level_up_message = font.render(f"Level Up! Now at Level {level}", True, (255, 255, 255))
    text_rect = level_up_message.get_rect(center=(screen_width // 2, screen_height // 2))
    screen.blit(level_up_message, text_rect)
...
def game_loop():
    ...
    while running:
        ...
        for i in range(num_of_enemies):
            ...
            if enemyX[i] <= 0:
                enemyX_change[i] = 1.5 + (level - 1) * 0.5
            ...
            if collision:
                ...
                if score_value >= level * level_threshold:
                    level_up()
                    level_up_time = pygame.time.get_ticks()  # Record the time when level up happens
        ...
        # Display level-up text if it's time
        current_time = pygame.time.get_ticks()
        if level_up_time and current_time - level_up_time < level_up_duration:
            level_up_text()
